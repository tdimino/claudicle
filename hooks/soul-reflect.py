...
Reads the JSONL transcript tail, extracts the last user→assistant exchange,
...
def _extract_last_exchange(transcript_path: str) -> tuple[str, str]:
    ...
    # Read last 50 lines (enough to find the last exchange)
    ...
    for line in tail:
        ...
        if msg_type == "human":
            # Extract text from human message content blocks
