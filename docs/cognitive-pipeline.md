...
`soul_engine.py` — `build_prompt()` generates a trace_id for the cognitive cycle, delegates context assembly to `context.py`, and appends cognitive step instructions. The trace_id groups all working_memory entries (decisions + cognitive outputs) from this cycle.

`context.py` — `build_context()` assembles the prompt context from these components, in order:
...
This is the **Samantha-Dreams pattern**: avoid redundant context injection while ensuring the model is available when the agent has learned something new. See `context.py` — `should_inject_user_model()`.
...
Defined in `soul_engine.STEP_INSTRUCTIONS` — a single dict of per-step instruction strings, assembled into the numbered block by `_assemble_instructions()`. Both unified mode and split mode use the same instruction registry.
...
`soul_engine.py` — `parse_response()` extracts XML-tagged sections from the LLM response. It consumes the trace_id generated by `build_prompt()`, ensuring decisions logged during context assembly and cognitive outputs share the same trace_id.
...
Each cognitive step uses a **verb** that reflects the soul's emotional state. Verbs are listed in `STEP_INSTRUCTIONS` in `soul_engine.py`.
...
All context-assembly decisions are logged to working_memory with `entry_type="decision"` and the cycle's trace_id. This makes gating decisions inspectable alongside cognitive outputs.

Logged decisions:
- **Skills injection** — "Inject skills reference?" (true on first message in thread)
- **User model injection** — "Inject user model?" (Samantha-Dreams gate)
- **Dossier injection** — "Inject dossiers?" (entity relevance matching, when DOSSIER_ENABLED)

### User Model Injection Gate

`context.py` — `should_inject_user_model()`, the Samantha-Dreams pattern:

```python
def should_inject_user_model(entries):
...
`soul_engine.py` — periodic gating via shared interaction counter in `context.py`:

```python
# In context.py — shared between unified and split modes:
...
|------|-------|-----------|
| Add cognitive steps | `soul_engine.STEP_INSTRUCTIONS` dict | `skills/open-souls-paradigm/references/additional-cognitive-steps.md` |
| Change context assembly | `context.py` — `build_context()` | `skills/open-souls-paradigm/references/memory-regions.md` |
| Change prompt instructions | `soul_engine.py` — `_assemble_instructions()` | `skills/open-souls-paradigm/references/additional-cognitive-steps.md` |
| Add per-step models | `pipeline.py` / `claude_handler.py` | `skills/open-souls-paradigm/references/multi-provider-models.md` |
...
| Query cognitive history | `working_memory.get_trace(trace_id)` | Self-inspection API for trace replay |
